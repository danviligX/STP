...
class LinearDMS_net(nn.Module):
    def __init__(self, trial=0):
        super(LinearDMS_net,self).__init__()
...
class LinearDMS_dataset(Dataset):
    def __init__(self,item_idx) -> None:
        ...
        for index in item_idx:
            ...
            track,_ = search_track_pos(meta_item,set_file[meta_item[0]],meta_item[1])
...
def LinearDMS_obj(trial):
    ...
    if data_div_method=='CV':
        ...
        for CV_i in range(data_div_para):
            ...
            train_loader,valid_loader = LinearDMS_data_loader(train_item_idx=train_item_idx,
...
def LinearDMS_test(net,test_loader,criterion):
